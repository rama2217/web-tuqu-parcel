...
<div class="w-full max-w-md">
    ...
    <p class="text-center text-sm mb-3" style="color:#C9A96E">Masuk ke akunmu</p>
    ...
    @if (session('error'))
        <div class="mb-5 p-4 rounded-xl text-sm text-red-700 bg-red-50 border border-red-200">
            {{ session('error') }}
        </div>
    @endif
    ...
    <div class="bg-white rounded-2xl shadow-lg p-8">
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-5">
            ...
            {{-- Password --}}
            <div>
                <label class="block text-sm font-medium mb-1.5" style="color:#2D4A3E">Password</label>
                <div class="relative">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                    <input type="checkbox" name="remember" id="remember"
                        ...
                        style="accent-color:#2D4A3E">
                    <label for="remember" class="text-sm text-gray-600 cursor-pointer">Ingat saya</label>
                </div>
                <a href="{{ route('password.request') }}" class="text-sm hover:underline" style="color:#C9A96E">Lupa password?</a>
            </div>
        ...
        <p class="text-center text-sm text-gray-500 mt-6">
            Belum punya akun?
            <a href="{{ route('register') }}" class="font-medium hover:underline" style="color:#C9A96E">Daftar sekarang</a>
        </p>
    </div>
